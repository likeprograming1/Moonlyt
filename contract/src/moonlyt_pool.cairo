...
#[starknet::interface]
trait MoonlytPoolTrait<TContractState> {
    ...
    fn get_Target(self: @TContractState) -> (u128 , MoonlytPool::targetOption) ;
    fn viewTarget(self: @TContractState) -> target;
    fn get_total_token_balance(self: @TContractState) -> u128;
}
...
mod MoonlytPool {
    ...
    #[external(v0)]
    impl MoonlytPoolImpl of super::MoonlytPoolTrait<ContractState> {
        ...
        fn get_total_token_balance(self: @ContractState) -> u128 {
            self.token.read().balanceOf(get_contract_address()).try_into().unwrap()
        }
    }
